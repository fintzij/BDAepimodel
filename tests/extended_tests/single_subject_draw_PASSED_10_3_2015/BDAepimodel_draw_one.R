...
epimodel <- init_epimodel(obstimes = seq(0, 10, by = 0.1),
                          popsize = popsize,
                          states = c("S", "I", "R"), 
                          params = c(beta = rnorm(1, 1, 1e-6), mu = rnorm(1, 1, 1e-6), rho = 0.5, p0 = 0.2), 
                          rates = c("beta * I", "mu"), 
...
epimodel$sim_settings <- init_settings(niter = 500000,
                                       burnin <- 0,
                                       params_every <- 1, 
                                       configs_every <- 1,
                                       kernel = kernel,
...
for(k in 1:.niter) {
         ...
         # resimulate the epidemic
          ...
          if(all(.epimodel$config_mat[,"S"] == .epimodel$popsize)) {
                   while(all(.epimodel$config_mat[,"S"] == .epimodel$popsize)) {
                             .epimodel <- simulate_epimodel(.epimodel, trim = FALSE, lump = TRUE)
                   } 
          }
          ...
          Gillespie_results[,k] <- ifelse(.epimodel$config_mat[which(.epimodel$config_mat[,"ID"] == 0),".X1"] == 2, 1, 0)
          
          # choose which subjects should be redrawn
          .subjects <- ifelse(sum(.epimodel$config_mat[1,.epimodel$.config_inds] == 2) == 1, which(.epimodel$config_mat[1,.epimodel$.config_inds] == 2), sample(which(.epimodel$config_mat[1,.epimodel$.config_inds] == 2), 1))
          ...
          # reflect the removal. 
          remove_trajectory(.epimodel, subject = .subjects[1], save_path = TRUE)
          
          # update instatiate missing IRMs, update the tpms 
          # and tpm products, the emission probability mtx,
          # and the FB matrices.
          update_matrices(.epimodel, subject = .subjects[1])
          
          # draw a new subject level trajectory
          draw_trajec(.epimodel, subject = .subjects[1])
          
          BDAepimodel_results[,k] <- ifelse(.epimodel$config_mat[which(.epimodel$config_mat[,"ID"] == 0),".X1"] == 2, 1, 0)
