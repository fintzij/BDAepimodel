...
epimodel <- init_epimodel(obstimes = seq(0, 10, by = 0.1),
                          popsize = popsize,
                          states = c("S", "I", "R"), 
                          params = c(beta = rnorm(1, 1, 1e-6), mu = rnorm(1, 1, 1e-6), rho = 0.5, S0 = 0.8, I0 = 0.2, R0 = 0), 
                          rates = c("beta * I", "mu"), 
...
epimodel$sim_settings <- init_settings(niter = 25000,
                                       save_params_every = 1, 
...
for(k in 1:niter) {
         ...
         # resimulate the epidemic
          ...
          if(.epimodel$config_mat[1,"I"] < 1) {
                   while(.epimodel$config_mat[1,"I"] < 1) {
                             .epimodel <- simulate_epimodel(.epimodel, trim = FALSE, lump = TRUE)
                   } 
          }
          ...
          Gillespie_results[,k] <- ifelse(.epimodel$config_mat[which(.epimodel$config_mat[,"ID"] == 0), paste0(".X",.subjects[1])] == 2, 1, 0)
          build_irm(.epimodel)
          ...
          BDAepimodel_results[,k] <- ifelse(.epimodel$config_mat[which(.epimodel$config_mat[,"ID"] == 0),paste0(".X", .subjects[1])] == 2, 1, 0)
