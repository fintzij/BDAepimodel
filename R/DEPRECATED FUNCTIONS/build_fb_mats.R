...
build_fb_mats <- function(fb_mats, epimodel) {
          ...
          for(m in 2:dim(epimodel$fb_mats)[3]) {
                    
                    fb_mats[,,m] <- normalize((colSums(fb_mats[,,m-1]) %*% t(epimodel$emission_mat[, m+1, drop = FALSE])) * epimodel$tpm_products[,,epimodel$obs_time_inds[m]])
