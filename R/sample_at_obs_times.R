...
sample_at_obs_times <- function(config_mat, epimodel, subject) {
        ...
        config_mat[epimodel$obs_time_inds, subject] <- state_vec
        ...
        if(epimodel$progressive & epimodel$absorbing_states) {
            ...
            if(any(state_vec %in% epimodel$absorbing_states)) {
                      ...
                      config_mat[epimodel$obs_time_inds[absorb_ind] : epimodel$ind_final_config, subject] <- state_vec[absorb_ind]
            }
            ...
            for(t in 1:(epimodel$nobs - 1)) {
                      ...
                      if(state_vec[t] == state_vec[t+1]) {
                                ...
                                config_mat[epimodel$obs_time_inds[t] : epimodel$obs_time_inds[t+1], subject] <- state_vec[t]
