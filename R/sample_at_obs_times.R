...
sample_at_obs_times <- function(epimodel, subject, subj_ID) {
          ...
          for(s in (epimodel$nobs - 1) : 1) {
                    .state_vec[s] <- .prev_state <- sample.int(n = epimodel$num_states, size = 1, prob = epimodel$.fb_mats[[s]][, .prev_state])
                    
          }
          
          # update the configuration matrix
          epimodel$config_mat[epimodel$.obs_time_inds, subj_ID] <- .state_vec
          ...
          if(epimodel$progressive & epimodel$absorbing_states) {
                    ...
                    for(t in 1:(epimodel$nobs - 1)) {
                              ...
                              if(.state_vec[t] == .state_vec[t+1]) {
                                        ...
                                        epimodel$config_mat[epimodel$.obs_time_inds[t] : epimodel$.obs_time_inds[t+1], subj_ID] <- .state_vec[t]
