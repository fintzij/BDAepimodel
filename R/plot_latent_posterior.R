...
plot_latent_posterior <- function(epimodel, states, times, cm = "med", proportion = FALSE, truth = NULL, true_popsize = NULL, dat = NULL) {
          ...
          if(!is.null(dat)) {
                    if(!proportion) {
                              trajecs_post <- trajecs_post + ggplot2::geom_point(ggplot2::aes(x = time, y = eval(parse(text = epimodel$meas_vars)), colour = paste(epimodel$model$meas_vars,"Observed Count")), size = 5, data = data.frame(epimodel$dat)) 
                    } else {
                              prop <- dat
