...
init_obs_mat <- function(epimodel){
          ...
          # if a config_mat is provided, compute the true counts at observation times
          if(!is.null(epimodel$config_mat)) {
                    for(k in 1:nrow(obs_mat)) {
                              obs_mat[k, paste(epimodel$meas_vars, "truth", sep="")] <- 
                                        epimodel$config_mat[, meas_vars][sum(epimodel$config_mat[,"time"] <= obs_mat[k, "time"])]
